...
get_cm <- function(in_mapping, exclude = NA) {
  ...
  if (!is.na(exclude)) {
    nullify_indices <- findOverlaps(cm_output@regions@ranges, exclude@ranges)@from
...
mcool2bedpe <- function(mcool, max_distance = NA, res = 1000) {
  ...
  in.interactions <- interactions(zoom(mcool,res))
...
# Define tvr loci
rmv7_tvr_locus_start <- 732217 - 6400*0.5
rmv7_tvr_locus_end <- 742163 + 6400*0.5
rmv7_tvr_locus <- GRanges(".57195_H01.1", IRanges(start=c(rmv7_tvr_locus_start),end=c(rmv7_tvr_locus_end)))
rmv8_tvr_locus_start <- 872458 - 6400*0.5
rmv8_tvr_locus_end <- 881034 + 6400*0.5
rmv8_tvr_locus <- GRanges("S_pneumoniae_RMV8_dominant", IRanges(start=c(rmv8_tvr_locus_start),end=c(rmv8_tvr_locus_end)))

# Output matrices
rmv7_porec_data <- list(get_cm(porec_rmv_mapping_list[[1]], exclude = rmv7_tvr_locus),
                         get_cm(porec_rmv_mapping_list[[2]], exclude = rmv7_tvr_locus),
                         get_cm(porec_rmv_mapping_list[[3]], exclude = rmv7_tvr_locus),
                         get_cm(porec_rmv_mapping_list[[4]], exclude = rmv7_tvr_locus),
                         get_cm(porec_rmv_mapping_list[[5]], exclude = rmv7_tvr_locus),
                         get_cm(porec_rmv_mapping_list[[6]], exclude = rmv7_tvr_locus)
                     )

rmv7_tvr_locus <- GRanges(1, IRanges(start=c(rmv7_tvr_locus_start),end=c(rmv7_tvr_locus_end)))

rmv7_hic_data <- list(get_cm(hic_rmv_mapping_list[[1]], exclude = rmv7_tvr_locus),
                         get_cm(hic_rmv_mapping_list[[2]], exclude = rmv7_tvr_locus),
                         get_cm(hic_rmv_mapping_list[[3]], exclude = rmv7_tvr_locus),
                         get_cm(hic_rmv_mapping_list[[4]], exclude = rmv7_tvr_locus),
                         get_cm(hic_rmv_mapping_list[[5]], exclude = rmv7_tvr_locus),
                         get_cm(hic_rmv_mapping_list[[6]], exclude = rmv7_tvr_locus)
                     )

rmv8_porec_data <- list(get_cm(porec_rmv_mapping_list[[7]],exclude = rmv8_tvr_locus),
                               get_cm(porec_rmv_mapping_list[[8]],exclude = rmv8_tvr_locus),
                               get_cm(porec_rmv_mapping_list[[9]],exclude = rmv8_tvr_locus),
                               get_cm(porec_rmv_mapping_list[[10]],exclude = rmv8_tvr_locus),
                               get_cm(porec_rmv_mapping_list[[11]],exclude = rmv8_tvr_locus),
                               get_cm(porec_rmv_mapping_list[[12]],exclude = rmv8_tvr_locus)
)

rmv8_tvr_locus <- GRanges(1, IRanges(start=c(rmv8_tvr_locus_start),end=c(rmv8_tvr_locus_end)))

rmv8_hic_data <- list(get_cm(hic_rmv_mapping_list[[7]],exclude = rmv8_tvr_locus),
                               get_cm(hic_rmv_mapping_list[[8]],exclude = rmv8_tvr_locus),
                               get_cm(hic_rmv_mapping_list[[9]],exclude = rmv8_tvr_locus),
                               get_cm(hic_rmv_mapping_list[[10]],exclude = rmv8_tvr_locus),
                               get_cm(hic_rmv_mapping_list[[11]],exclude = rmv8_tvr_locus),
                               get_cm(hic_rmv_mapping_list[[12]],exclude = rmv8_tvr_locus)
)
...
generate_cm_plot <- function(index,matrix_list = NULL, res = NULL, df = NULL) {
  ...
  HiContacts::plotMatrix(zoom(matrix_list[[index]],res),
                         caption = FALSE) +
    xlab('') +
...
``` {r Focus on off-diagonal points}

off_diagonal_lowres_interactions_bedpe_df <-
  lores_interactions_bedpe_df %>% 
  dplyr::mutate(IF = dplyr::if_else(abs(start1 - start2) <= lores_value*2+1,1,IF))

(off_diagonal_interaction_summary_plot <-
  ggplot(dplyr::bind_rows(
            off_diagonal_lowres_interactions_bedpe_df,
            off_diagonal_lowres_interactions_bedpe_df %>%
              dplyr::filter(start1 != start2) %>%
...
ggsave("./supplementary_figures/Offdiagonal_LoRes_PoreC.png",
       height = 5,
       width = 8,
       dpi = 600)
...
get_distance_from_origin <- function(df,var,origin,genome_length) {
  ...
  df %>% 
    dplyr::mutate(
      ...
      option4 = (!!var_col_name - !!origin_col_name),
    ) %>% 
    dplyr::mutate(Distance_from_origin =
                    dplyr::case_when(
                      abs(option1) <= abs(option2) & abs(option1) <= abs(option3) & abs(option1) <= abs(option4) ~ option1,
                      abs(option2) <= abs(option1) & abs(option2) <= abs(option3) & abs(option2) <= abs(option4) ~ option2,
                      abs(option3) <= abs(option1) & abs(option3) <= abs(option2) & abs(option3) <= abs(option4) ~ option3,
                      abs(option4) <= abs(option1) & abs(option4) <= abs(option2) & abs(option4) <= abs(option3) ~ option4               
                    )
                  ) %>%
...
interactions_by_origin_df <-
  hires_interactions_bedpe_df %>% 
              ...
              dplyr::mutate(start1 = get_distance_from_origin(hires_interactions_bedpe_df,start1,Origin,Length),
                            start2 = get_distance_from_origin(hires_interactions_bedpe_df,start2,Origin,Length),
                            end1 = get_distance_from_origin(hires_interactions_bedpe_df,end1,Origin,Length),
                            end2 = get_distance_from_origin(hires_interactions_bedpe_df,end2,Origin,Length)
                            )
...
cowplot::plot_grid(
  ...
  ncol = 1,
  labels = "AUTO",
  rel_heights = c(0.2,0.35,0.3,0.3)
)
...
gwas_locus_size <- 1600

rmv7.data.com <- mergeCMs(get_cm(zoom(porec_rmv_mapping_list[[1]],res=gwas_locus_size), exclude = rmv7_tvr_locus),
                         get_cm(zoom(porec_rmv_mapping_list[[2]],res=gwas_locus_size), exclude = rmv7_tvr_locus),
                         get_cm(zoom(porec_rmv_mapping_list[[3]],res=gwas_locus_size), exclude = rmv7_tvr_locus),
                         get_cm(zoom(porec_rmv_mapping_list[[4]],res=gwas_locus_size), exclude = rmv7_tvr_locus),
                         get_cm(zoom(porec_rmv_mapping_list[[5]],res=gwas_locus_size), exclude = rmv7_tvr_locus),
                         get_cm(zoom(porec_rmv_mapping_list[[6]],res=gwas_locus_size), exclude = rmv7_tvr_locus)
                     )
...
rmv7.hic.data.com <- mergeCMs(get_cm(zoom(hic_rmv_mapping_list[[1]],res=gwas_locus_size), exclude = rmv7_tvr_locus),
                         get_cm(zoom(hic_rmv_mapping_list[[2]],res=gwas_locus_size), exclude = rmv7_tvr_locus),
                         get_cm(zoom(hic_rmv_mapping_list[[3]],res=gwas_locus_size), exclude = rmv7_tvr_locus),
                         get_cm(zoom(hic_rmv_mapping_list[[4]],res=gwas_locus_size), exclude = rmv7_tvr_locus),
                         get_cm(zoom(hic_rmv_mapping_list[[5]],res=gwas_locus_size), exclude = rmv7_tvr_locus),
                         get_cm(zoom(hic_rmv_mapping_list[[6]],res=gwas_locus_size), exclude = rmv7_tvr_locus)
                     )
...
colnames(normalised.rmv7.hic.data.y) <- c("domi_1","domi_2","domi_3","rare_1","rare_2","rare_3")
...
unfiltered_rmv7_porec_hicexp <- make_hicexp(data_list = rmv7_porec_bedpe_list, 
                                           ...
                                           A.min = 0, #32, # 40
                                           filter = TRUE,
                                           remove.regions = rmv7_tvr_locus)

rmv7_porec_diffhic_filtering_threshold <- 8
...
rmv7_hic_bedpe_list <- lapply(hic_rmv_mapping_list[1:6],mcool2bedpe, res = gwas_locus_size, max_distance = 10)
...
rmv7_hic_diffhic_filtering_threshold <- 64 # was 256 for 6.4 kb
...
rmv8.data.com <- mergeCMs(get_cm(zoom(porec_rmv_mapping_list[[7]],res=gwas_locus_size), exclude = rmv8_tvr_locus),
                         get_cm(zoom(porec_rmv_mapping_list[[8]],res=gwas_locus_size), exclude = rmv8_tvr_locus),
                         get_cm(zoom(porec_rmv_mapping_list[[9]],res=gwas_locus_size), exclude = rmv8_tvr_locus),
                         get_cm(zoom(porec_rmv_mapping_list[[10]],res=gwas_locus_size), exclude = rmv8_tvr_locus),
                         get_cm(zoom(porec_rmv_mapping_list[[11]],res=gwas_locus_size), exclude = rmv8_tvr_locus),
                         get_cm(zoom(porec_rmv_mapping_list[[12]],res=gwas_locus_size), exclude = rmv8_tvr_locus)
                     )
...
rmv8.hic.data.com <- mergeCMs(get_cm(zoom(hic_rmv_mapping_list[[7]],res=gwas_locus_size), exclude = rmv8_tvr_locus),
                         get_cm(zoom(hic_rmv_mapping_list[[8]],res=gwas_locus_size), exclude = rmv8_tvr_locus),
                         get_cm(zoom(hic_rmv_mapping_list[[9]],res=gwas_locus_size), exclude = rmv8_tvr_locus),
                         get_cm(zoom(hic_rmv_mapping_list[[10]],res=gwas_locus_size), exclude = rmv8_tvr_locus),
                         get_cm(zoom(hic_rmv_mapping_list[[11]],res=gwas_locus_size), exclude = rmv8_tvr_locus),
                         get_cm(zoom(hic_rmv_mapping_list[[12]],res=gwas_locus_size), exclude = rmv8_tvr_locus)
                     )
...
unfiltered_rmv8_porec_hicexp <- make_hicexp(data_list = rmv8_porec_bedpe_list, 
                                             ...
                                             A.min = 0, # 32, #40,
                                             filter = TRUE,
                                             remove.regions = rmv8_tvr_locus)


rmv8_porec_diffhic_threshold_val <- 8 # was 16
...
rmv8_hic_diffhic_threshold_val <- 64 # was 1024
...
(combined_manhattan_plot <-
    ggplot(combined_differential_df,
         aes(x = locus,
             ...
             colour = Genotype,
             shape = Data)) +
    geom_vline(aes(xintercept = start), colour = "red", linetype = 2, alpha = 0.5) +
    geom_vline(aes(xintercept = end), colour = "red", linetype = 2, alpha = 0.5) +
...
prci_logFC.df <-
  combined_differential_df %>% 
    dplyr::filter(locus >= start & locus <= end) %>% 
    dplyr::rowwise() %>% 
  ...
  ggplot(prci_contacts_df %>% 
           dplyr::mutate(
             `Distance classification` = 
               dplyr::case_when(
                 Distance < 200 ~ "<200 bp",
                 Distance < 400 ~ "200-399 bp",
                 Distance < 1000 ~ "400-999 bp",
                 TRUE ~ ">1000 bp"
               )
           ) %>% 
...
rmv7_ivr_locus_start <- 2102757
rmv7_ivr_locus_end <- 2112955
rmv8_ivr_locus_start <- 527250
rmv8_ivr_locus_end <- 535325
...
add_distance_classification <- function(input_df, resolution = NULL) {
  ...
  lower_bound <- paste0("≤",resolution," bp")
  mid_bound <- paste0(resolution+1,"-1000 bp")
  ...
  input_df %>%
    dplyr::mutate(
      distance_classification = 
        factor(
          dplyr::case_when(
            abs(Position - Position_Y) <= resolution ~ lower_bound,
            abs(Position - Position_Y) <= 1000 ~ mid_bound,
            abs(Position - Position_Y) <= 2000 ~ "1001-2000 bp",
            TRUE ~ ">2000 bp"
          ),
          levels = c(lower_bound,mid_bound,"1001-2000 bp",">2000 bp")
        )
...
matrix_resolution <- 400

# RMV7 characteristics
hic.rmv7_characteristics_df <-
  read.csv("./sample_info/res400.S_pneumoniae_RMV7_dominant_characteristics.csv")

# RMV8 characteristics
hic.rmv8_characteristics_df <-
  read.csv("./sample_info/res400.S_pneumoniae_RMV8_dominant_characteristics.csv")

# RMV7 HiC dataset
rmv7_hic_cm <- 
  dplyr::bind_rows(
    dplyr::bind_rows(
      sparse2tibble(get_cm(zoom(hic_rmv_mapping_list[[4]],matrix_resolution))),
      sparse2tibble(get_cm(zoom(hic_rmv_mapping_list[[5]],matrix_resolution))),
      sparse2tibble(get_cm(zoom(hic_rmv_mapping_list[[6]],matrix_resolution))),
      .id = "Replicate"
    ) %>%
    dplyr::mutate(Variant = "RMV7_rare"),
    
    dplyr::bind_rows(
      sparse2tibble(get_cm(zoom(hic_rmv_mapping_list[[1]],matrix_resolution))),
      sparse2tibble(get_cm(zoom(hic_rmv_mapping_list[[2]],matrix_resolution))),
      sparse2tibble(get_cm(zoom(hic_rmv_mapping_list[[3]],matrix_resolution))),
      .id = "Replicate"
    ) %>%
  ...
  dplyr::filter(!(Position >= rmv7_tvr_locus@ranges@start & Position <= rmv7_tvr_locus@ranges@start+rmv7_tvr_locus@ranges@width))

rmv8_hic_cm <- 
  dplyr::bind_rows(
    dplyr::bind_rows(
      sparse2tibble(get_cm(zoom(hic_rmv_mapping_list[[7]],matrix_resolution))),
      sparse2tibble(get_cm(zoom(hic_rmv_mapping_list[[8]],matrix_resolution))),
      sparse2tibble(get_cm(zoom(hic_rmv_mapping_list[[9]],matrix_resolution))),
      .id = "Replicate"
    ) %>%
    dplyr::mutate(Variant = "RMV8_domi"),
    
    dplyr::bind_rows(
      sparse2tibble(get_cm(zoom(hic_rmv_mapping_list[[10]],matrix_resolution))),
      sparse2tibble(get_cm(zoom(hic_rmv_mapping_list[[11]],matrix_resolution))),
      sparse2tibble(get_cm(zoom(hic_rmv_mapping_list[[12]],matrix_resolution))),
      .id = "Replicate"
    ) %>%
  ...
  add_positions(char_df = hic.rmv8_characteristics_df,
                matrix_resolution = matrix_resolution) %>%
  add_distance_classification(resolution = matrix_resolution) %>%
  dplyr::filter(!(Position >= rmv8_tvr_locus@ranges@start & Position <= rmv8_tvr_locus@ranges@start+rmv8_tvr_locus@ranges@width))
...
matrix_resolution <- 200
# RMV7 characteristics
porec.rmv7_characteristics_df <-
  read.csv("./sample_info/res200.S_pneumoniae_RMV7_dominant_characteristics.csv")

# RMV8 characteristics
porec.rmv8_characteristics_df <-
  read.csv("./sample_info/res200.S_pneumoniae_RMV8_dominant_characteristics.csv")

rmv7_porec_cm <- 
  dplyr::bind_rows(
    dplyr::bind_rows(
      sparse2tibble(get_cm(zoom(porec_rmv_mapping_list[[4]],matrix_resolution))),
      sparse2tibble(get_cm(zoom(porec_rmv_mapping_list[[5]],matrix_resolution))),
      sparse2tibble(get_cm(zoom(porec_rmv_mapping_list[[6]],matrix_resolution))),
      .id = "Replicate"
    ) %>%
    dplyr::mutate(Variant = "RMV7_rare"),
    
    dplyr::bind_rows(
      sparse2tibble(get_cm(zoom(porec_rmv_mapping_list[[1]],matrix_resolution))),
      sparse2tibble(get_cm(zoom(porec_rmv_mapping_list[[2]],matrix_resolution))),
      sparse2tibble(get_cm(zoom(porec_rmv_mapping_list[[3]],matrix_resolution))),
      .id = "Replicate"
    ) %>%
    dplyr::mutate(Variant = "RMV7_domi")
  ) %>%
...
rmv8_porec_cm <- 
  dplyr::bind_rows(
    dplyr::bind_rows(
      sparse2tibble(get_cm(zoom(porec_rmv_mapping_list[[10]],matrix_resolution))),
      sparse2tibble(get_cm(zoom(porec_rmv_mapping_list[[11]],matrix_resolution))),
      sparse2tibble(get_cm(zoom(porec_rmv_mapping_list[[12]],matrix_resolution))),
      .id = "Replicate"
    ) %>%
    dplyr::mutate(Variant = "RMV8_rare"),
    
    dplyr::bind_rows(
      sparse2tibble(get_cm(zoom(porec_rmv_mapping_list[[7]],matrix_resolution))),
      sparse2tibble(get_cm(zoom(porec_rmv_mapping_list[[8]],matrix_resolution))),
      sparse2tibble(get_cm(zoom(porec_rmv_mapping_list[[9]],matrix_resolution))),
      .id = "Replicate"
    ) %>%
    dplyr::mutate(Variant = "RMV8_domi")
  ) %>%
...
(hic_re_sites_plot <-
 ggplot(re_sites_df,
  ...
  facet_grid(Method~Genotype+Enzyme) +
  ylab('Frequency') +
  theme_bw() +
  theme(strip.background = element_blank())
)
...
process_input_df <- function(input_df, genome_length = NULL, position_of_origin = NULL , method = NULL) {
  ...
  dplyr::mutate(mean_GC = (GC+GC_Y)/200,
                mean_MluCI = (MluCI+MluCI_Y)/2,
                mean_NlaIII = (NlaIII+NlaIII_Y)/2
                ) %>%
  dplyr::rowwise() %>%
...
Zoomed in view of PRCI[malA].

``` {r Detailed view of PRCI_malA}

ggplot(hires_interactions_bedpe_df %>%
        dplyr::filter(Genotype == "RMV8") %>% dplyr::mutate(distance_classification = dplyr::if_else(Distance <= 400,"Small","Large")),
        aes(x = start1,
            y = IF)
       ) +
     geom_point() +
     #scale_y_log10() +
     xlim(c(prci_malA_start-10000,prci_malA_end+10000)) +
     facet_grid(Method+Variant~distance_classification, scales = 'free_y') +
     theme_bw()

```

Zoomed in view of PRCI[dnaN].

``` {r Detailed view of PRCI_dnaN}

ggplot(hires_interactions_bedpe_df %>%
        dplyr::filter(Genotype == "RMV7") %>% dplyr::mutate(distance_classification = dplyr::if_else(Distance <= 400,"Small","Large")),
        aes(x = start1,
            y = IF)
       ) +
     geom_point() +
     #scale_y_log10() +
     xlim(c(prci_dnaN_start-10000,prci_dnaN_end+10000)) +
     facet_grid(Method+Variant~distance_classification, scales = 'free_y') +
     theme_bw()
...
fit_glmmTMB_structure <- function(input_df) {
  ...
  contact_density_model <-
    glmmTMB::glmmTMB(
      ContactFrequency ~ distance_classification + mean_GC + mean_MluCI + mean_NlaIII + Classification*Variant + Distance_from_origin*Variant + (Variant|Position) + (1|Replicate),# + (1|Position:Variant),
      data = input_df %>%
                dplyr::mutate(Position = factor(Position)) %>% 
                dplyr::mutate(mean_GC = 10*mean_GC),
      family = poisson(link = "log"),
      ziformula = ~ distance_classification*mean_MluCI + distance_classification*mean_NlaIII + mean_GC,
...
distance_palette <-
  c(
    "≤200 bp" = "turquoise",
    "≤400 bp" = "aquamarine",
    "201-1000 bp" = "lightblue",
    "401-1000 bp" = "cornflowerblue",
    "1001-2000 bp" = "blue",
    ">2000 bp" = "navy"
  )
...
plot_prediction <- function(model) {
  ...
  input_df <-
    ...
    dplyr::rename(`Distance classification` = distance_classification)
...
```{r Combine prediction plots, height = 12, width = 8}
...
legend_plot <-
  ggplot(
    tibble::tibble(
      ...
      dplyr::mutate(`Distance classification` = factor(`Distance classification`,
                                                       levels = c("≤200 bp","≤400 bp","201-1000 bp","401-1000 bp","1001-2000 bp",">2000 bp"))),
...
```{r Compare coefficient estimates}
...
glmer_model_summary_df <-
  ...
  dplyr::mutate(term = dplyr::if_else(
                    term == "distance_classification201-1000 bp" | term == "distance_classification401-1000 bp",
                    "distance_classificationPeak intensity-1000 bp",
                    term
                  )
                ) %>% 
...
(distance_plot <-
  ggplot(glmer_model_summary_df %>%
           dplyr::filter(grepl("distance_",term) & !grepl(":",term)) %>%
           dplyr::mutate(Distance = gsub(' bp','',gsub("distance_classification","",term))) %>%
           dplyr::mutate(Distance = 
                           factor(dplyr::case_when(
                                      Distance == "Peak intensity-1000" ~ "Peak-\n1000",
                                      Distance == "1001-2000" ~ "1000-\n2000",
                                      TRUE ~ Distance,
                                  ),
                                  levels = c("Peak-\n1000",
                                             "1000-\n2000",
    ...
    xlab('Separation between loci (bp)') +
    theme_bw() +
    theme(strip.background = element_blank())
)
...
(re_site_plot <-
  ggplot(glmer_model_summary_df %>%
           dplyr::filter(component == "Poisson" & (grepl("mean_MluCI",term) | grepl("mean_NlaIII",term)) |
                           component == "Zero inflated" & (term == "mean_MluCI" | term == "mean_NlaIII")) %>% 
...
(mge_summary_plot <-
  ggplot(glmer_model_summary_df %>%
           dplyr::filter(grepl("PRCI",term) | grepl("Tn916",term) | grepl("prophage",term)) %>%
           dplyr::mutate(Effect =
...
```{r Plot differences per site}

position_ranef_df <-
  dplyr::bind_rows(
    ranef(rmv7_hic_glmer_model)$cond$Position %>%
      dplyr::mutate(Method = "Hi-C",
                    Genotype = "RMV7",
                    Model = "RMV7 Hi-C"),
    ranef(rmv7_porec_glmer_model)$cond$Position %>%
      dplyr::mutate(Method = "Pore-C",
                    Genotype = "RMV7",
                    Model = "RMV7 Pore-C"),
    ranef(rmv8_hic_glmer_model)$cond$Position %>%
      dplyr::mutate(Method = "Hi-C",
                    Genotype = "RMV8",
                    Model = "RMV8 Hi-C"),
    ranef(rmv8_porec_glmer_model)$cond$Position %>%
      dplyr::mutate(Method = "Pore-C",
                    Genotype = "RMV8",
                    Model = "RMV8 Pore-C")
  ) %>%
  ...
  dplyr::mutate(Position = as.integer(gsub('\\.+[1234567890]+','',Position))) %>%
  dplyr::mutate(Difference = dplyr::if_else(
                    !is.na(VariantRMV7_rare),
                    VariantRMV7_rare,
                    VariantRMV8_rare
                  )
                ) %>% 
  dplyr::select(-c(VariantRMV7_rare,VariantRMV8_rare))

(distance_correlation_plot <-
  ggplot(position_ranef_df %>%
           dplyr::mutate(Experiment = factor(paste(Genotype,Method),
                                             levels = c("RMV7 Hi-C",
                                                        "RMV8 Hi-C",
                                                        "RMV7 Pore-C",
                                                        "RMV8 Pore-C"))
    ...
    # scale_x_continuous(trans = "log10") +
    # scale_y_continuous(trans = "log10") +
    ylab(expression('Contact frequency in rare variant')) +
    xlab(expression('Contact frequency in dominant variant')) +
    scale_colour_manual(values = genotype_palette) +
    scale_shape_manual(values = method_shapetypes) +
    theme_bw() +
    geom_smooth() +
...
mge_detail_df <-
  dplyr::bind_rows(
      # PRCI_uvrA
      position_ranef_df %>% 
        dplyr::filter(Genotype == "RMV7" & Position >= 1792101 & Position <= 1804961) %>% 
        dplyr::mutate(
      ...
      # PRCI_dnaN
      position_ranef_df %>% 
        dplyr::filter(Genotype == "RMV7" & Position >= 1601016 & Position <= 1619555) %>% 
        dplyr::mutate(
      ...
      # PRCI_mal
      position_ranef_df %>% 
        dplyr::filter(Genotype == "RMV8" & Position >= 1998066 & Position <= 2010058) %>% 
        dplyr::mutate(
...
correlation_output_df <-
  ...
  dplyr::mutate(Separation = dplyr::if_else(Method == "Hi-C",Interval*500,Interval*400))
...
intervariant_across_tech_df <-
  dplyr::bind_rows(
    position_ranef_df %>% 
      ...
      dplyr::select(Position,Value,Genotype,Difference) %>% 
      dplyr::rename(HiC_value = Value) %>% 
      ...
      dplyr::left_join(
        position_ranef_df %>% 
          ...
          dplyr::select(Position,Value,Genotype,Difference) %>% 
          dplyr::rename(PoreC_value = Value) %>% 
...
bottom_plot <-
  cowplot::plot_grid(
    plotlist = list(
      lagged_correlation_plot + theme(legend.position = "none"),
      method_intervariant_comparison_plot + theme(legend.position = "none")
    ),
    nrow = 1,
    rel_widths = c(0.5,0.5),
    labels = c("F","G")
...
figure_3_legend <-
  cowplot::get_plot_component(distance_plot + theme(legend.position = "bottom"), 'guide-box-bottom', return_all = TRUE)
...
rna_induction_df <-
  process_qpcr_input_file("./qPCR/induction_MMC_RMV8_sugar.csv",
...
legend_df <-
  tibble::tibble(
    ...
    "Genotype" = c("RMV8[domi]","RMV8[rare]","RMV7[domi]","RMV7[rare]","RMV8[domi]~italic(tvr)*'::'*italic(cat)","RMV8[rare]~italic(tvr)*'::'*italic(cat)")
  )
...
get_growth_legend <- function(plot) {
  ...
  pl <-
    cowplot::get_plot_component(plot  + theme(legend.position = "right") + guides(fill = g,
                                                                                  colour = g),
                            'guide-box-right',
                            return_all = TRUE)
